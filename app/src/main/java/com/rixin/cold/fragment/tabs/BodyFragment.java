...
public class BodyFragment extends BaseFragment {
        ...
        MyRecyclerView myRecyclerView = new MyRecyclerView(manager, mAdapter) {
            ...
            public void onRefresh() {
                if (NetworkUtils.isNetworkConnected(UIUtils.getContext())) {
                    mData = getServiceData(GlobalConstants.TABS_BODY_URL);
            ...
            public void onLoadMore() {
                if (NetworkUtils.isNetworkConnected(UIUtils.getContext())) {
                    page += 1;
                    ArrayList<ColdInfo> moreData = getServiceData(GlobalConstants.TABS_BODY_NEXT_URL + page);
                    if (moreData != null) {
    ...
    public LoadingPage.ResultState onLoadData() {
        ...
        if (beforeTime.equals(getCurrentDate())) {
            // 如果日期相符则加载缓存
            mData = CacheUtils.getCache(GlobalConstants.TABS_BODY_CACHE_KAY);
            if (mData == null) {
                mData = getServiceData(GlobalConstants.TABS_BODY_URL);
                // 写缓存
                CacheUtils.setCache(GlobalConstants.TABS_BODY_CACHE_KAY, getCacheString(mData));
            }
        } else {
            mData = getServiceData(GlobalConstants.TABS_BODY_URL);
            // 写缓存
